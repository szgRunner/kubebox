  ...
  constructor({ screen, navbar, status, client }) {
    ...
    function updatePodsTable() {
      ...
      pods_table.setData((pods_list.items || []).reduce((rows, pod) => {
        ...
        rows.push(row);
        return rows;
      }, [['NAME', 'NODE', 'STATUS', 'RESTARTCOUNT', 'AGE']]));
    ...
    this.podRun = function (namsepace, pod) {
      pod_selected = pod;
      current_namespace = namsepace;

      let listPodsError;
      // FIXME: should be cancellable
      const promise = until(client.pod(current_namespace, pod_selected).get())
        .do(pods_table, pods_table.setLabel)
        .spin(s => `${s} Pods {grey-fg}[${current_namespace}]{/grey-fg}`)
        .succeed(_ => `Pods {grey-fg}[${current_namespace}]{/grey-fg}`)
        .then(response => {
          pods_list = JSON.parse(response.body.toString('utf8'));
          pods_list.items = [];
          if (pods_list.items.length > 0) {
            pods_list.items = pods_list.items;
          } else {
            pods_list.items.push(pods_list);
          }
          pods_list.items = pods_list.items || [];
          updatePodsTable();
        }).catch(error => {
