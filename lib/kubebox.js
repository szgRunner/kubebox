...
const { Dashboard, login, namespaces, pods, NavBar, spinner: { until }, setContent } = require('./ui/ui');
...
class Kubebox extends EventEmitter {
  ...
  constructor(screen, server) {
    ...
    const { log } = require('./ui/debug')(screen);
    const CORS = isWebBrowser && !server;

    let kube_config, current_namespace, current_pod;
    ...
    screen.key(['n'], () => {
      namespaces.prompt(screen, client, { current_namespace })
        .then(namespace => {
          ...
          log(`Switching to namespace ${current_namespace}`);
          screen.render();
    ...
    // when the connection is established to the cluster
    screen.key(['p'], () => {
      pods.promptPod(screen, client, {current_namespace})
        .then(pod => {
          dashboard.reset(page);
          // switch dashboard to new namespace
          screen.render();
          return dashboard.podRun(current_namespace, pod);
        })
        .catch(error => console.error(error.stack));
    });
    ...
    function connect(login, options = {}) {
      if (login) log(`Connecting to ${client.url} ...`);
      const { promise, cancellation } = client.api().get({ cancellable: true });
      cancellations.add('connect', () => {
        if (cancellation()) log(`Cancelled connection to ${client.url}`);
      });
      return until(promise
        ...
        .then(() => login ? log(`Connected to ${client.url}`) : '')
